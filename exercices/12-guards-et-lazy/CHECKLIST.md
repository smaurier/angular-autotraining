...
- [ ] `AuthService` est cree avec les signaux `isLoggedIn` et `isAdmin`
- [ ] `AuthService` expose les methodes `login()`, `logout()` et `toggleAdmin()`
...
- [ ] `adminGuard` verifie a la fois l'authentification et le role admin
...
- [ ] `EditProductComponent` detecte les modifications (signal `isDirty`)

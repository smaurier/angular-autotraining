...
Créer un catalogue de produits compose de deux composants : un parent qui géré la liste et un enfant qui affiche chaque produit, en utilisant `input()` et `output()`.

## Consignes

1. Créer deux fichiers dans `src/app/exercises/ex05/` :
   - `product-catalog.component.ts` (composant parent)
   - `product-card.component.ts` (composant enfant)
2. Définir une **interface** `Product` :
   - `id` : `number`
   - `name` : `string`
   - `price` : `number`
   - `description` : `string`
...
3. **Composant enfant `ProductCardComponent`** :
   ...
   - Emettre un événement `addToCart` via `output<Product>()` quand on clique sur "Ajouter au panier"
...
4. **Composant parent `ProductCatalogComponent`** :
   ...
   - Créer un computed `cartCount` : nombre d'articles dans le panier
   - Créer un computed `cartTotal` : prix total du panier
   - Iterer avec `@for` sur les produits et afficher un `<app-product-card>` par produit
   - Gérer l'événement `addToCart` pour ajouter le produit au panier
5. Afficher un résumé du panier en haut de page
...
- Ajouter une quantite au panier (éviter les doublons, incrementer la quantite)

...
- [ ] La méthode `addTodo()` ajoute une tache et vide le champ de saisie
- [ ] La méthode `toggleTodo(id)` inverse le statut completed avec `update()` (immutabilite)
- [ ] La méthode `deleteTodo(id)` supprime une tache avec `update()` + `filter()`
